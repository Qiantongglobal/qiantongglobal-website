...
//current login user
router.get('/employees/login/user', async (req, res) => {
    try {
        const user = req.user;
...
router.get('/users', async (req, res) => {
    try {
        ...
        const provider = createContactsProvider(req);
        const result = await provider.listUsers(offset ? parseInt(offset) : undefined, limit ? parseInt(limit) : undefined);
        res.json({ success: true, data: result });
    }
    catch (error) {
...
router.post('/users', async (req, res) => {
    try {
        ...
        if (!input.corp_id)
            input.corp_id = req.user.corp_id;
        const provider = createContactsProvider(req);
